...
Preference writes use strict recipient type, channel, and boolean validation. Timed mute is supported with `muted=true` plus an ISO-8601 `mutedUntil`; unmuting clears `mutedUntil`. Quiet hours require a complete `quietHoursStart` / `quietHoursEnd` / IANA `timezone` tuple. Push created inside the recipient-local quiet window remains `handoff_ready` but is deferred through `scheduledAt` until the quiet window ends; claim acquisition excludes future-scheduled plans. Overnight windows are supported and equal start/end values are rejected.
