...
Internal notification APIs are service-authorized. The Host supplies trusted `notifying_service_key` and `notifying_service_scopes` request attributes; standalone `dev`/`test` runtime may use `X-Notifying-Service-Key` and `X-Notifying-Service-Scopes`. Production never trusts those headers. Intent ingestion requires `intent:write`; dispatch inspection, claim, handoff, fail, and cancel require `dispatch:consume`. Caller-supplied `claimedBy` is only an assertion against the verified service identity and cannot select the lease owner.

Preference writes use strict recipient type, channel, and boolean validation. Timed mute is supported with `muted=true` plus an ISO-8601 `mutedUntil`; unmuting clears `mutedUntil`. Quiet hours require a complete `quietHoursStart` / `quietHoursEnd` / IANA `timezone` tuple. Indefinite mute is a hard push suppression, while timed mute defers push instead of suppressing it. Timed mute and quiet hours compose: if mute expiry lands inside quiet hours, `scheduledAt` advances to the quiet-hours end. Deferred push remains `handoff_ready` with reason `push-policy-deferred`, and claim acquisition excludes future-scheduled plans. Overnight windows are supported and equal start/end values are rejected.
...
Delivering must claim work before handoff. Claim acquisition uses an atomic conditional database update from `handoff_ready` to `claimed`, so concurrent workers cannot successfully claim the same row. Each claim batch receives one opaque `claimLeaseId`; only its SHA-256 hash is persisted with `claimedBy`, `claimedAt`, and `claimExpiresAt`. The raw lease ID is returned only by the claim response and must be carried by the consumer into `/handoff`. A failure transition from the `claimed` state likewise requires both `claimedBy` and the current `claimLeaseId`. Expired claims are recovered back to `handoff_ready` before the next claim scan and receive a new lease identity when reclaimed, preventing a stale worker from completing or failing a newer claim even when the worker name is reused.
